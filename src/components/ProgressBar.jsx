...
export default function ProgressBar({ 
  ...
  const getProgressStatus = () => {
    if (totalTasks === 0) return { status: 'empty', color: 'slate', message: 'No tasks yet' };
    if (progressPercentage === 100) return { status: 'complete', color: 'green', message: 'Completed!' };
    if (progressPercentage >= 75) return { status: 'near-complete', color: 'blue', message: 'Almost done' };
    if (progressPercentage >= 50) return { status: 'in-progress', color: 'indigo', message: 'Making progress' };
    if (progressPercentage >= 25) return { status: 'started', color: 'purple', message: 'Getting started' };
  ...
  return (
    <div className={`${className}`}>
      {showDetails && (
        <div className={`px-4 py-3 rounded-lg border ${backgroundClasses[color]} mb-3`}>
        ...
        <div className={`w-full bg-slate-200 rounded-full ${sizeClasses[size]} overflow-hidden`}>
          <div 
            ...
            style={{ 
              ...
              boxShadow: progressPercentage > 0 ? '0 0 10px rgba(99, 102, 241, 0.3)' : 'none'
            }}
          />
        </div>
